class CollectionsController < ApplicationController
  ...
  def show
    compare = @collection.assets.where.not(image_url: nil).where('updated_at < ?', Time.current - 3.minutes).order(Arel.sql("(1 - (votes_count :: DECIMAL / MAX((votes_count+1) * 100) OVER())) * (floor(random() * 40 + 1) :: int) desc")).limit(25)
